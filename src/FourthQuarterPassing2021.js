...
const isTeamName = (value) => 'ARI' || 'ATL' || 'BAL' || 'BUF' || 'CAR' || 'CHI' || 'CIN' || 'CLE' || 'DAL' || 'DEN' || 'DET'
|| 'DET' || 'GB' || 'HOU' || 'IND' || 'JAX' || 'KC' || 'LA' || 'LAC' || 'LV' || 'MIA' || 'MIN' || 'NE' || 'NO'
...
const separateWithCommas = (statYear) => statYear.replace(/\s+/g, ',');

separateWithCommas(FourthQuarterPassing2021);
